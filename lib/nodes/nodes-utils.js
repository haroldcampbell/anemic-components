...
export function addArcShapeNode(parentId, id) {
    ...
    a.$radialCenter = () => __radialCenter(a);
 ...
 *
 * WARNING: Depends on $calcRenderData() being called first.
 *
 * @param {ArcShapeNode} node
 *
 * @return {Object} - Hash with the {x,y} as keys.
 */
function __radialCenter(node) {
    const start = node.__arcStart();
    const end = node.__arcEnd();
